...
node {
   ...
   parallel (
     "default-build": {
       stage('\u27A1 Build') {
          configFileProvider(
              [configFile(fileId: 'sonatype-settings', targetLocation: 'settings.xml')]) {
                sh "'${mvnHome}/bin/mvn' -s settings.xml clean deploy -Ddocumentation.dir=${WORKSPACE} -Psordocs,sonatype -U"
          }
       }
       stage('\u27A1 Results') {
          archive '**/target/*.jar'
