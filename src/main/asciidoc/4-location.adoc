...
It's also possible to modify and save an existing `Location`. Therefore the client sends a `PUT` request with the full required
representation of the `Location`, including the necessary ID fields.

include::{generated}/loc-updated/http-request.adoc[]

include::{generated}/loc-updated/request-fields.adoc[]

If the server has saved the `Location` successfully the response looks like:
