...
Locking a `Target` for Inbound/Outbound operation is controlled with a HTTP POST request against the URI `/v1/targets/ALLOCATION_LOCK` with
...
Currently the API offers to variants to create a `TransportUnit`. Both require a HTTP POST request sent to the same context path but with
...
The server stores a `TransportUnit` instance with the given values and default values for other fields of the `TransportUnit`. The response
contains a link to the newly created resource.
...
Notice the `strict` attribute. When the client passes `true` as value for `strict` the server ensures there is no `TransportUnit` with the
given business key. When the attribute is omitted or set to `false` no such validation happens and the reference to an already existing
`TransportUnit` or the newly created is returned. If the API call fails because a `TransportUnit` with the given `Barcode` exists, the
response looks like:
...
Location where the `TransportUnit` shall be booked on.
...
During the handling of `TransportUnits`, errors or failure situations may occur that make it necessary to store some kind of error message
...
client simple calls this API to store a new error message for an existing `TransportUnit`.
